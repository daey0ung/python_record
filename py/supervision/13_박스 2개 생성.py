...
zone1 = sv.PolygonZone(polygon=polygon1) # polygon zone 생성
zone2 = sv.PolygonZone(polygon=polygon2) # polygon zone 생성
polygon_annotator1 = sv.PolygonZoneAnnotator(
    zone=zone1,
...
polygon_annotator2 = sv.PolygonZoneAnnotator(
    zone=zone2,
...
def process_frame(frame):
    ...
    # [수정] polygon zone 트리거 선언 및 프레임에 polygon zone 표시 / 2개 처리
    is_detections_in_zone1 = zone1.trigger(detections) # 트리거를 활성화하고 zone에 있는지 상태를 저장한다
    print(is_detections_in_zone1)
    is_detections_in_zone2 = zone2.trigger(detections)  # 트리거를 활성화하고 zone에 있는지 상태를 저장한다
    annotated_frames = polygon_annotator1.annotate(annotated_frames) # polygon zone1을 프레임에 표시
    annotated_frames = polygon_annotator2.annotate(annotated_frames) # polygon zone2을 프레임에 표시

    # [수정] polygon에 있는 객체의 tracker_id값을 저장해서 화면에 표시 / 2개 처리
    in_polygon_list1 =[]
    for tracker_id, is_in_polygon in zip(detections.tracker_id, is_detections_in_zone1):
        if is_in_polygon:
            in_polygon_list1.append(int(tracker_id))
    in_polygon_list1.sort() # 숫자 정령
    cv2.putText(
        annotated_frames, f"in_polygon = {in_polygon_list1}" , (500, 60), cv2.FONT_HERSHEY_COMPLEX, 2,
        (255, 255, 255), 2, cv2.LINE_AA)

    in_polygon_list2 = []
    for tracker_id, is_in_polygon in zip(detections.tracker_id, is_detections_in_zone2):
        if is_in_polygon:
            in_polygon_list2.append(int(tracker_id))
    in_polygon_list2.sort()  # 숫자 정령
    cv2.putText(
        annotated_frames, f"in_polygon = {in_polygon_list2}", (500, 150), cv2.FONT_HERSHEY_COMPLEX, 2,
