...
box_annotator = sv.BoxAnnotator(thickness=2, color=sv.Color(r=255, g=0, b=0))
...
def process_frame(frame):
    ...
    low_speed = [] # 낮은 속도의 차량 트래커 아이디 저장 리스트
    ...
    is_zone_and_speed = is_detections_in_zone & high_speed_mask # polygon과 과속 차량의 마스크 중복값만 True 시킴

    # 프레임에 객체 박스 표시 / is_zone_and_speed에서 True인 값만 박스처리
    annotated_frame = box_annotator.annotate(frame.copy(), detections[is_zone_and_speed])  # frame.copy(): 프레임을 훼손하지 않고 복사본에서 진행
