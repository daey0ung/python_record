...
# 과속차량 박스와 저속차량 박스
high_box_annotator = sv.BoxAnnotator(thickness=2, color=sv.Color(r=255, g=0, b=0))
low_box_annotator = sv.BoxAnnotator(thickness=2, color=sv.Color(r=0, g=0, b=255))
...
# [추가] 사진 찍힌 트래커는 제외하는 리스트
saved_traker_id = []

def process_frame(frame):
    """비디오 영상의 프레임을 처리하는 함수"""

    global frames_in_sec, t0, fps_text, saved_traker_id # FPS 관련 변수 선언

    results = model(frame, verbose=False, conf=0.35)[0] #, imgsz=1088
    ...
    # pts_in = points[is_detections_in_zone] if len(points) else points  # points에 값이 있으면 마스크처리 없으면 그대로
    # print(is_detections_in_zone)

    # 과속 저속 변수 생성
    high_speed = [] # 과속차량 트래커 아이디 저장 리스트
    high_speed_mask = [] # 과속차량 마스크
    low_speed = [] # 저속차량 트래커 아이디 저장 리스트
    low_speed_mask = [] # 저속차량 마스크
    ...
    for tracker_id, [_,y] in zip(detections.tracker_id, points):
        ...
        if len(coordinates[tracker_id]) < video_info.fps/2:
            ...
            if int(speed) > 80: # 속도가 80이상이면
                high_speed.append(int(tracker_id)) # 트래커 아이디 저장
            elif int(speed) < 20: # 속도가 20 이하이면
                low_speed.append(int(tracker_id)) # 트래커 아이디 저장

            labels.append(f'#{tracker_id} {int(speed)} km/h')

    # 과속,저속 차량 마스크 만들기
    for tracker_id in detections.tracker_id:
        if tracker_id in high_speed: # 과속차량 트래커 아이디 리스트에 트래커 아이디가 있다면
            high_speed_mask.append(True) # 과속차량 마스크값을 True
            low_speed_mask.append(False)
        elif tracker_id in low_speed:  # 저속차량이라면
            high_speed_mask.append(False)
            low_speed_mask.append(True)  # 저속차량 마스크값을 True
        else: # 과속차량 리스트에 없다면
            high_speed_mask.append(False) # 과속차량 마스크값을 False
            low_speed_mask.append(False)

    # print(is_detections_in_zone)
    # print(high_speed_mask)
    is_zone_and_high_speed = is_detections_in_zone & high_speed_mask # polygon과 과속 차량의 마스크 중복값만 True 시킴
    is_zone_and_low_speed = is_detections_in_zone & low_speed_mask  # polygon과 저속 차량의 마스크 중복값만 True 시킴

    # 프레임에 객체 박스 표시 / is_zone_and_speed에서 True인 값만 박스처리
    annotated_frame = high_box_annotator.annotate(frame.copy(), detections[is_zone_and_high_speed])  # 과속차량 박스처리
    annotated_frame = low_box_annotator.annotate(annotated_frame, detections[is_zone_and_low_speed])  # 저속차량 박스처리

    cv2.polylines(annotated_frame, [polygon], isClosed=True, color=(255, 255, 255), thickness=3) # 프레임에 polygon은 opencv를 이용하여 표시

    # [추가] 파일명에 적을 시간값
    time_sec = datetime.now().replace(microsecond=0).strftime("%Y%m%d%H%M%S")

    # [추가] 과속차량 캡쳐
    for idx, (box, tracker_id) in enumerate(zip(detections[is_zone_and_high_speed].xyxy, detections[is_zone_and_high_speed].tracker_id)):
        print(tracker_id)
        print(saved_traker_id)
        if int(tracker_id) not in saved_traker_id:
            x1, y1, x2, y2 = box.astype(int)
            crop_frame = frame.copy()[y1:y2, x1:x2]
            file_name = f'high_speed_crop/{time_sec}_{tracker_id}.png'
            cv2.imwrite(file_name, crop_frame)
            saved_traker_id.append(int(tracker_id))
    ...
    for box, label in zip(detections[is_detections_in_zone].xyxy, labels):
        ...
        cv2.putText(
            annotated_frame, label, (x1, y1 - 10), fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=1,
    ...
    cv2.putText(annotated_frame, fps_text, (30, 60), # frame: 글씨를 적을 프레임, fps_text: 적을 문자열, (10,24): 문자열의 시작위치(좌측 하단 모서리 좌표),
