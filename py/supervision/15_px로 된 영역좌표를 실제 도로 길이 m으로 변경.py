...
def process_frame(frame):
    ...
    # [추가] 탐지 객체들의 하단 중심점을 points에 저장
    points = detections.get_anchors_coordinates(anchor=sv.Position.BOTTOM_CENTER)
    # print(points)

    # [추가] 탐지 객체들의 하단 중심점 리스트값이 도로 실제 길이(x,y)로 변경되는 과정
    if points.size > 0:
    ...
    # [수정] 프레임에 객체 박스, 객체 경로 표시 / polygon zone 안에 있는 탐지 객체들만 표시(mask기능을 사용)
